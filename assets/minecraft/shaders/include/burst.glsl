...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if((nCoord.y > -(line_width / 2.0) - y_adjuster && nCoord.y < (line_width / 2.0) - y_adjuster)){
      ...
      if(nCoord.x < -line_dis-(line_long/4.0) - x_adjuster  && nCoord.x > -line_dis-(line_long/2.0) - x_adjuster ){
        fragColor = vec4(vec3(0.0), 0.0);
      }
      if(nCoord.x > line_dis+(line_long/4.0) - x_adjuster  && nCoord.x < line_dis+(line_long/2.0) - x_adjuster ){
        fragColor = vec4(vec3(0.0), 0.0);
      ...
      if(nCoord.y> line_dis - y_adjuster && nCoord.y < line_dis + line_long - y_adjuster){
        fragColor = vec4(vec3(1.0), 1.0);
      }else if(nCoord.y < -line_dis - y_adjuster && nCoord.y > -(line_dis + line_long) - y_adjuster){
        fragColor = vec4(vec3(1.0), 1.0);
      }
      if(nCoord.y < -line_dis-(line_long/4.0) - y_adjuster && nCoord.y > -line_dis-(line_long/2.0) - y_adjuster){
        fragColor = vec4(vec3(0.0), 0.0);
      }
      if(nCoord.y > line_dis+(line_long/4.0) - y_adjuster && nCoord.y < line_dis+(line_long/2.0) - y_adjuster){
        fragColor = vec4(vec3(0.0), 0.0);
